...
class OpenAppFromTaskbarPortraitExplicitTest {
    @Rule
    @JvmField
    ...
    companion object {
        ...
        fun setUp() {
            ...
            traceReader = withTracing {
                // Launch browser app from taskbar
    ...
    @Test
    fun assertBrowserIsVisibleAtEnd() {
        finalState.isVisible(browserApp)
    }

    @Test
    fun assertBrowserIsInsideDisplayBoundsAtEnd() {
        finalState.visibleRegion(browserApp)
            .coversAtMost(wmHelper.currentState.wmState.getDefaultDisplay()!!.displayRect)
    }

    @Test
    fun assertBrowserIsLaunchedWithCascading() {
        val displayAppBounds = WindowUtils.getInsetDisplayBounds(rotation)
        val windowBounds = finalState.visibleRegion(browserApp).region.bounds
